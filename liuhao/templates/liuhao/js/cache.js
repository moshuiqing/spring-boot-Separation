...
layui.use(['form', 'jquery', "layer"], function() {
	...
	function showNotice() {
		layer.open({
			...
			btn: ['关闭'],
			moveType: 1,
			content: '<div style="padding:15px 20px; text-align:justify; line-height: 22px; text-indent:2em;border-bottom:1px solid #e2e2e2;"><p class="layui-red">欢迎登陆！</p></div>',
	...
	if (headImg == null || headImg == '') {
		headImg = webip + webname + "/images/face.jpg";
	} else {
		headImg = webip + headImg;
	}
	...
	function lockPage() {
		shuopin=layer.open({
			...
			content: '<div class="admin-header-lock" id="lock-box">' +
				'<div class="admin-header-lock-img"><img src="' + headImg + '" class="userAvatar"/></div>' +
			...
			success: function() {
				...
				//判断是否设置过头像，如果设置过则修改顶部、左侧和个人资料中的头像，否则使用默认头像
				var face = window.sessionStorage.getItem('userFace');
				if (face != null && face != "null" && face != '' && $(".userAvatar").length > 0) {

					$(".userAvatar", window.parent.document).attr('src', webip + window.sessionStorage.getItem('userFace'));
				} else {
					$(".userAvatar").attr("src", webip + webname + "/images/face.jpg");
				}
	...
	$("body").on("click", "#unlock", function() {
		if ($(this).siblings(".admin-header-lock-input").val() == '') {
			...
			$.post(webname + "/backsystem/sysuser/sysLogin", param, function(d) {
				if (qjty(d)) {
					if (d.code > 0) {
	...
	$(".functionSetting").click(function() {
	  gongneng=layer.open({
			...
			success: function(index, layero) {
				...
				$(".functionSrtting_box input[name=changeRefresh]").prop("checked", changeRefreshStr == "true" ? true :
					false);
				...
				form.on("submit(settingSuccess)", function(data) {
					...
					var isSingle = data.field.oneLogin == "on" ? "1" : "0";
					$.post(webname + "/backsystem/main/isSingle", {
						isSingle: isSingle,
						id: id
					}, function(d) {
						if (qjty(d)) {
