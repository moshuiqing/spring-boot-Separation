   ...
   $.fn.Flowdesign = function(options)
    ...
    if(processData.list)
    {
        $.each(processData.list, function(i,row) 
        {
            ...
            .attr("process_to",row.processto)
            .attr("process_id",row.id)
            .addClass("process-step btn btn-small")
            .html('<span class="process-flag badge '+badge+'"><i class="'+icon+' icon-white"></i></span>&nbsp;' + row.processname )
    ...
    jsPlumb.bind("click", function(c) {
    	var index=layer.confirm('你确定取消连接吗?',{title:"提示",icon: 3, btn: ['确定',  '取消'] },function(){
    		 jsPlumb.detach(c);
    		 layer.close(index);
    ...
    var _canvas_design = function(){
        ...
        $('.process-step').each(function(i){
            ...
            var prcsto = $(this).attr('process_to');
			 var toArr="";
			if(prcsto!=null && prcsto!='' && prcsto!=undefined){
				toArr = prcsto.split(",");
			}
            var processData = defaults.processData;
    ...
    var Flowdesign = {
    	...
    	upProcess:function(row){
       	...
       	//使可以连接线
            ...
            ////************************************************************************************
            if(row.processtoCopy!=null){
            var copy = row.processtoCopy;
            var copyarry = copy.split(",");
           for (var int = 0; int < copyarry.length; int++) {
        	   
        	   var conn = jsPlumb.getConnections({
                   //only one of source and target is needed, better if both setted
               		source:"window"+row.id, 
                    target:"window"+copyarry[int]
